...
从 GitHub 下载 ZIP 解压后，根据系统双击对应文件即可，**无需提前安装任何软件**：
...
脚本会自动完成：
1. 检测操作系统
2. 检查 Node.js — 未安装或版本过低会自动安装 Node.js 20.x
   - Windows: 通过 `winget` 或下载 MSI 安装包
   - macOS: 通过 Homebrew（未安装 Homebrew 也会自动安装）
   - Linux: 通过系统包管理器（apt / yum / dnf / pacman / apk）
3. 安装 npm 依赖
4. 启动服务并自动打开浏览器 → http://localhost:3000

> **Windows 注意**：首次安装 Node.js 后需要**关闭窗口再双击一次** `install-windows.bat`（让系统 PATH 生效）。
>
> **macOS 首次双击提示"无法打开"或"无法验证开发者"**：
>
> 方法一（推荐）：打开 **系统设置 → 隐私与安全性** → 底部找到被阻止的文件 → 点击"仍要打开"
>
> 方法二：**右键点击** `install-macos.command` → 选择"打开" → 点击"打开"
>
...
```bash
bash install-linux.sh --service    # 安装为 systemd 系统服务（开机自启）
bash install-linux.sh --docker     # Docker 构建并运行
bash install-linux.sh --uninstall  # 卸载 systemd 服务
bash install-linux.sh --help       # 查看帮助

PORT=8080 bash install-linux.sh              # 自定义端口
NPM_MIRROR=cn bash install-linux.sh          # 使用国内 npm 镜像
```
...
> **快捷方式**：Linux 下可直接运行 `bash install-linux.sh --docker`，一键构建并启动 Docker 容器。
...
> **快捷方式**：`bash install-linux.sh --service` 可一键完成安装 + 注册 systemd 服务 + 开机自启。
